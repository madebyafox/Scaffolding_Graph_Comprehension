...
<br>
<!--html_preserve-->
<img src="Study3_files/figure-html/expected_results.png" height="500" width="500" > 
<!--/html_preserve-->
...
The results of a 3(explicit-scaffold) X 2(implicit-scaffold) factorial ANOVA reveal significant main effects of explicit-scaffold and (F = 49.5, p < 0.001) implicit-scaffold (F = 9.9, p < .005) as well as a significant interaction beteen explicit & implicit scaffolds (F = 1.3, p < .005).  
...
mainEffectGraph  <- t.test(triangular_score ~ impasse, data=df_subjects)
mainEffectGraph
...
desc_control <- by(df_subjects$triangular_score, list(df_subjects$impasse), stat.desc, basic=FALSE)
desc_control
...
Post-hoc Tukey tests reveal signifiant differences between each level of the explicit scaffolds, and both levels of impasse problem.    

Within the non-impasse condition, each explicit scaffold was significantly different than the others, in the predicted order (control condition lowest, then text/image scaffold, then interactive image)   
1-1 <*** 2-1 p = 0.0007   
2-1 <**  3-1 p = 0.0063   
<br> 
Counter to expectations, within the impasse condition, the control-explicit scaffold was significantly different than the others   
1-2 <*** 2-2 p = 0.0006   
1-2 <*** 3-2 p = 0.0000006   
however the difference between the interactive and text/image scaffolds under impasse condition were non-signficant, as expected.   
2-2 < 3-2   p = 0.5578632   
<br> 
We also predicted that the impasse-conditions would be significantly better than the non-impasse scaffolds   
N 1-1 < 1-2  --> not significant    
N 2-1 < 1-2  --> not significant   
N 3-1 > 1-2  p = 0.0000168  --> counter   
Y 1-1 < 2-2  p = 0.0000000   
N 2-1 < 2-2  p = 0.0970521 --> approaching significance   
Y 3-1 < 2-2  --> not significant   
N 3-1 < 3-2  --> not significant   
<br> 
