...
```{r collapse=TRUE}
#explore descriptive statistics based on each IV
desc <- by(df_subjects$score_diff, list(df_subjects$condition), stat.desc, basic=FALSE)
desc
# LM_mean = desc_control$linear_score[2]
# LM_SD = desc_control$linear_score[6]
# TM_mean = desc_control$triangular_score[2]
...
```{r} 
bar <- ggplot(df_subjects, aes(x = condition, y= score_diff))
...
simpleModel = ezANOVA(data = df_subjects,
                     dv = .(score_diff),
                     wid = .(subject),
                     between = .(condition),
                     type = 3,
...
pairwise.t.test(df_subjects$score_diff, df_subjects$condition,  p.adjust.method="bonferroni") 
...
# model.1=aov(triangular_score~linear_score+condition, data=df_subjects)
# 
...
test<-lm(formula = triangular_score ~ linear_score + condition,  data = df_subjects)
summary(test)
```
...
table(df_subjects$`Graph Type`, df_subjects$condition)
chisq.test(df_subjects$`Graph Type`, df_subjects$condition, correct=TRUE) 
fisher.test(df_subjects$`Graph Type`, df_subjects$condition) 
...
table(df_subjects$`Graph Type`, df_subjects$order)
chisq.test(df_subjects$`Graph Type`, df_subjects$order, correct=TRUE) 
fisher.test(df_subjects$`Graph Type`, df_subjects$order) 
# ftable(bigTable)
```
...
#SETUP DATA FOR RESPONSE ACCURACY ANALYSIS
df_small_draw <- subset(df_subjects, df_subjects$`Graph Type`%in% c("Triangular (asymmetric)","Triangular (right angle)"))
...
<!--html_preserve--><a href="gallery/analysis.html">See GALLERY of all images here</a><!--/html_preserve-->
